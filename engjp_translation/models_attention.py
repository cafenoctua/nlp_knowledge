...
class LuongAttetion:
    def __init__(self, units=300):
        ...
        self.attetion = Activation(activation='softmax', name='attetion')
    ...
    def __call__(self, enc_output, dec_output):
        attetion = self.dot([dec_output, enc_output])
        attetion_weight = self.attetion(attetion)
        context_vector = self.context([attetion_weight, enc_output])
...
class AttentionDecoder(Decoder):
    ...
    def build(self):
        ...
        return Model(inputs=[self.input, self.enc_output, self.state_input],
                    outputs=[decoder_output, decoder_state])
